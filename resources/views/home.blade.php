...
<nav class="nav flex flex-wrap items-center justify-center px-4">
    <div class="flex flex-no-shrink items-center mr-24 py-3 text-grey-darkest">
        ...
        <span class="font-semibold text-xl tracking-tight">Word Phone</span>
    </div>
    ...
    <ul class="menu border-b md:border-none flex justify-end list-reset m-0 w-full md:w-auto">
        ...
        <li class="border-t md:border-none">
            <a href="#"
                class="block md:inline-block px-3 py-3 no-underline text-grey-darkest hover:text-grey-darker"> Catálogo</a>
        </li>

        <li class="border-t md:border-none">
            <a href="#"
                class="block md:inline-block px-3 py-3 no-underline text-grey-darkest hover:text-grey-darker">Sobre Nosotros</a>
        </li>
    </ul>
    ...
    <div style="margin-left: 3rem">
        ...
        @auth
            <button id="avatarButton" class="p-1 bg-blue-200 rounded-full focus:outline-none focus:ring">
            ...
            <div id="modal"
                class="hidden absolute mt-3 transform -translate-x-full bg-white rounded-md shadow-lg min-w-max z-20">
                ...
                <div class="p-2 hover:bg-blue-100 cursor-pointer">
                    <a href="{{ route('dashboard') }}" class="inline-flex items-center py-3 text-gray-800 " :class="{'justify-start': menu, 'justify-center': menu == false}">
                ...
                <div class="p-2">
                    <button class="flex items-center justify-center py-3 text-gray-800 border-t w-full css-button-sharp--black  ">
...
{{-- modal login --}}
<div style="z-index: 40" id="loginModal"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 mt-6 gap-7 md:gap-6 items-center">
        <div class="flex flex-col justify-center items-center md:items-start md:pr-15">
            <div class="flex items-center justify-center mb-4 md:justify-start">
                <h4 class="ml-2 text-sm font-bold tracking-widest text-primary uppercase">Lorem ipsum dolor sit amet sit
                    amet</h4>
            </div>
            <h1
                            ...
                            <tbody>
                                @foreach ($productos as $key => $producto)
                                    <tr>
                                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                            <div class="flex">
                                                <div class="flex-shrink-0 w-4 h-8">
                                                    <img class="w-full h-full " src="{{ asset('img/phone.png') }}"
                                                        alt="phone">
                                                </div>
                                                <div class="ml-3">
                                        ...
                                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                            <p class="text-gray-600 whitespace-no-wrap">{{ $producto->Descripción }}
                                            </p>
                                        </td>
                                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                        ...
                                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                            <span
                                                ...
                                                <span
                                                    id="statusLabel{{ $key }}">{{ $producto->getStockAttribute()->label() }}</span>
                                            </span>
                                        </td>
                                            ...
                                            <div id="modalstatus{{ $key }}"
                                                ...
                                                <div class="flex flex-col items-center p-4">
                                                    <div class="flex items-center mb-4">
                                                        ...
                                                        <input id="counterInput" type="number" min="0"
                                                            value="{{ $producto->inventario->CantidadDisponible
                                                            }}"
                                                            class="px-3 py-1 bg-gray-100 text-center"
                                                            style="width: 70px;">

                                                    </div>
                                                    <button id="changeStatus{{ $key }}"
                                                        class="px-4 py-2 bg-blue-500 text-white rounded">Cambiar
                                                        estado</button>
                                                </div>
                                            </div>
...
<script>
    ...
    var cantidadDisponible = {{ $producto->inventario->CantidadDisponible }};
    $("#counterInput").val(cantidadDisponible);

    // Función para actualizar el estado del producto y el estilo del statusLabel
    function actualizarEstado() {
        if (cantidadDisponible === 0) {
            $("#statusLabel{{ $key }}").text("Agotado").removeClass("text-green-800").addClass("text-red-800");
        } else {
            $("#statusLabel{{ $key }}").text("Disponible").removeClass("text-red-800").addClass("text-green-800");
        }
    }
    ...
    $("#decrementCounter").click(function() {
        if (cantidadDisponible > 0) {
            cantidadDisponible--;
            $("#counterInput").val(cantidadDisponible);
            actualizarEstado();
    ...
    // Manejo del evento click para el botón de cambio de estado
    $("#changeStatus{{ $key }}").click(function() {
        actualizarEstado();
        $("#modalstatus{{ $key }}").addClass("hidden");
    });
});
